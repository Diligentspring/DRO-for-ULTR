...
class policy_IPS_PBM(BaseAlgorithm):
    ...
    * Thorsten Joachims, Adith Swaminathan, Tobias Schnahel. 2017. Unbiased Learning-to-Rank with Biased Feedback. In Proceedings of WSDM '17
    ...
    def __init__(self, feature_size, exp_settings):
        ...
        # self.exams = [1.0 / (k+1) for k in range(10)]
        # self.exams = torch.tensor([10, 1, 0.513224915, 0.319571749,	0.223224116, 0.150037274, 0.117189338,	0.084112562, 0.048536682, 0.023450103, 0.01]) # Regression-EM
        # self.exams = torch.tensor([10, 1, 0.618775402, 0.32627172, 0.201888933, 0.124923788, 0.096322107, 0.080455341, 0.053930185, 0.041582826, 0.01])  # DLA_PBM
        # self.exams = torch.tensor([10, 1, 0.650218889, 0.435363861, 0.415684155, 0.268941877, 0.204243087, 0.179461963, 0.137051168, 0.092793092, 0.01])  # PBM_click_model
        self.exams = torch.tensor(
            [1000000, 1, 0.6738, 0.4145, 0.2932, 0.2079, 0.1714, 0.1363, 0.1166, 0.0838, 0.0579])  # All-pair

        if self.is_cuda_avail:
            self.exams = self.exams.to(device=self.cuda)

        with open('/home/niuzechun/ULTRA_DRO/selection_bias/query_doc_exam_dic', 'rb') as f:
            self.query_doc_exam_dic = pickle.load(f)

        # for k in self.query_doc_exam_dic.keys():
        #     print(self.query_doc_exam_dic[k])
        #     break
    ...
    def train(self, input_feed):
        ...
        propensity_values = self.exams

        qids = input_feed["qids"]
        docs = input_feed["docs"]
        ...
        for i in range(len(qids)):
            ...
            for d in this_docs:
                if d != '-1':
                    # print(qid)
                    # print(d)
                    this_propensities.append(self.query_doc_exam_dic[qid][d])
                else:
                    this_propensities.append(1)
            propensities.append(this_propensities)
        self.propensity = torch.tensor(propensities).to(device=self.cuda)
        ...
        if self.hparams.loss_func == 'sigmoid_loss':
            self.loss = self.sigmoid_loss_on_list(
                train_output, train_labels, self.propensity_weights)
        ...
        if not is_online_simulation:
            ...
            for metric in self.exp_settings['metrics']:
                ...
                for topn, metric_value in zip(topn, metric_values):
                    self.create_summary('%s_%d' % (metric, topn),
                                        '%s_%d' % (metric, topn), metric_value.item(), False, is_validation=True)
