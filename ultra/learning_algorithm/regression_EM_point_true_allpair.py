...
class RegressionEM_point_true_allpair(BaseAlgorithm):
    ...
    * Cappé, Olivier, and Eric Moulines. "Online expectation–maximization algorithm for latent data models." Journal of the Royal Statistical Society: Series B (Statistical Methodology) 71.3 (2009): 593-613.
    ...
    def __init__(self, feature_size, exp_settings):
        ...
        """
        print('Build Regression-based EM algorithm with true pointwise loss function and allpair propensities.')
        ...
        with torch.no_grad():
            self.propensity = torch.tensor([1, 1, 0.6738, 0.4145, 0.2932, 0.2079, 0.1714, 0.1363, 0.1166, 0.0838,
                                            0.0579])  # All-pair reproducibility
            if self.is_cuda_avail:
        ...
        # self.optimizer_func = torch.optim.AdamW(self.model.parameters(), lr=self.hparams.learning_rate)
