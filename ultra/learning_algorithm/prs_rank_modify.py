...
class PRSrank_modify(BaseAlgorithm):
    ...
    def __init__(self, feature_size, exp_settings):
        ...
        #     [0.000001, 1, 0.65, 0.42, 0.3, 0.21, 0.18, 0.15, 0.12, 0.08, 0.06]) #All-pair myself
        self.exams = torch.tensor(
            [0.000001, 1, 0.6738, 0.4145, 0.2932, 0.2079, 0.1714, 0.1363, 0.1166, 0.0838, 0.0579])  # All-pair reproducibility
    ...
    def train(self, input_feed):
        ...
        #
        #     model_dir = self.exp_settings['model_dir'].strip().split('/')[1]
        #     train_data_prefix = self.exp_settings['train_data_prefix']
        #     pw_file = open('./{}/pw/{}_exam.txt'.format(model_dir, train_data_prefix), 'r')
        #
        #     exams = pw_file.readline().strip('\n').split(' ')
        #     exams = [max(float(exam), 0.01) for exam in exams]
        #     if self.rank_list_size > len(exams):
        #         for _ in range(self.rank_list_size - len(exams)):
        #             exams.append(exams[-1])
        #     exams = exams[:self.rank_list_size]
        #     pw_list = [1.0 / float(exam) for exam in exams]
        #
        #     pw_file.close()
        #
        #     ipw.append(torch.as_tensor(pw_list).to(device=self.cuda))
        #
        #     # for l in range(self.rank_list_size):
        ...
        ipw = torch.ones_like(self.propensity) / self.propensity

        pw = self.propensity + torch.zeros_like(self.propensity)
        # pw = ultra.utils.metrics._safe_div(torch.tensor(1.0).to(device=self.cuda), ipw)
        ...
        training_output = self.ranking_model(self.model,
                                          self.rank_list_size)
        # training_output = torch.nan_to_num(training_output_raw)  # the output of the ranking model may contain nan
