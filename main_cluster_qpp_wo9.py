...
def get_qpp_cluster(info, index):
    ...
    for j in range(len(positions)):
        if positions[j] <= 10:
            vectors.append(np.array(info['query_document_embedding'][j], dtype=np.float16))
...
def get_batch_click(model, data_set, batch_size, list_size, index):
    ...
    for _ in range(batch_size):
        ...
        batch_clusters = get_qpp_cluster(data_set[i], index)
...
def train(exp_settings):
    ...
    with open('/home/niuzechun/ULTRA_DRO/Kmeans/centroids_cls_qpp_c50.txt', 'rb') as f:
        centroids = pickle.load(f)

    # print(centroids)

    # index = faiss.IndexFlatL2(12)
    index = faiss.IndexFlatL2(768*2)
    index.add(centroids)
